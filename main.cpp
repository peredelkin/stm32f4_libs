    ...
    if (TIM8->DIER & TIM_DIER_CC1IE) { // Capture Interrupt Enable Check
        ...
        if (TIM8->SR & TIM_SR_CC1IF) { // Check capture flag
            ...
            if (TIM8->CR1 & TIM_CR1_CEN) { //Checking if timer started
                ...
                cap_last = cap_cur; // Write last capture
                
                cap_cur = (uint16_t) TIM8->CCR1; // Read current capture
                
                TIM8->CCR3 = (uint16_t) (0xffff + cap_cur); // Set "Stop"
                
                capture = 65536 + cap_cur - cap_last; // Calc actual capture
                
                mark = (capture * 2)+(capture / 2); // Calc "Mark"
                
                if (mark >= 0xffff) { // Check for overflow "Mark"
                    
                    TIM8->CCR2 = (uint16_t) (0xffff + cap_cur); // Set "Mark" (!) Возможно не нужно
                    
                    TIM8->DIER &= ~TIM_DIER_CC2IE; // "Mark" Interrupt Disable
                    
                } else {
                    
                    TIM8->CCR2 = (uint16_t) (((uint16_t) mark) + cap_cur); //Write "Mark"
